 ...
 * USBD_FAIL
 */
extern QueueHandle_t xPcCommsQueue;
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t* Len) {
  ...
  if(*Len < 3) return USBD_OK;

  uint8_t header[HeaderSize];
  Debugger_Downstream downstreamMsg = Debugger_Downstream_init_zero;
  pb_istream_t stream = pb_istream_from_buffer(Buf, *Len);

  bool status = true;

  while(stream.bytes_left > 2){
  pb_read(&stream, &header[0], 1);
  pb_read(&stream, &header[1], 1);

  // not really nice to do this here but for now lets leave it :)
  if(header[0] == MagicBytes[0] && header[1] == MagicBytes[1]){
      status = pb_decode_ex(&stream, Debugger_Downstream_fields, &downstreamMsg, PB_DECODE_DELIMITED);

      if(status){
        xQueueSendFromISR(xPcCommsQueue, &downstreamMsg, &xHigherPriorityTaskWoken);
      }
    }
  }
