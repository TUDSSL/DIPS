...
#define ERROR_IF_NO_TARGET()	\
	if(!cur_target) { target_lost(); break; }
...
unsigned int times_intermittent = 0;
bool m_interrupt_on_power_failure = false;
arch_mode_t m_current_architecture = UNKNOWN_ARCH;
operation_mode_t m_debugger_mode = MODE_REGULAR;
bool disable_supply_after_next_continue = false;
...
Again:
    if(m_debugger_mode != MODE_PROFILE) disable_supply();
    ...
    switch (reason) {
        case TARGET_HALT_BREAKPOINT:
            ...
            if(m_current_architecture != MSP_ARCH){
                // Not sure why this is implemented but it is unnessecary for the MSP arch
                platform_timeout_set(&timeout, 200);
                while ((reason = target_halt_poll(cur_target, &watch)) == TARGET_HALT_BREAKPOINT){
            ...
            if (reason == TARGET_HALT_BREAKPOINT){
                ...
                break;
            } else if (reason != TARGET_HALT_INTERMITTENT){
                goto Again;
            }
            /* Fallthrough for TARGET_HALT_INTERMITTENT */
        case TARGET_HALT_INTERMITTENT:
            if(m_debugger_mode != MODE_PROFILE) disable_supply();
...
int gdb_main_loop(struct target_controller *tc, bool in_syscall)
{
   ...
   while (1) {
       // When device was intermittent:
       if (!target_available()){
           // Reconnect procedure:
           // Check if device is reachable again or return
           // Reconnect:
           // Send SWD->JTAG command
           // Compare IDCODE with old IDCODE when different -> exception
           // Turn on Debug Port
           // Get AP en DP information from cache
           // Write AP select and unlock the AP's
           // Halt and Enable Supply
           // Reconfigure breakpoints
           // Continue execution + Disable Supply

           // Take note of the frees and memory allocations
           // Maybe when device is deattached, free more rather then when connection is lost
           target_list_free();
           ...
           if(m_debugger_mode == MODE_PROFILE){
              disable_supply_after_next_continue = true;
           }
           ...
           // retransmit breakpoints/watchpoints
           target_restore_breakpoints(cur_target);
           if (!do_halt && !m_interrupt_on_power_failure){
...
static void handle_z_packet(char *packet, const size_t plen)
{
   ...
   int ret = 0;
   if (packet[0] == 'Z')
       ret = target_breakwatch_set(cur_target, type, addr, len, true);
   else
