...
#include "serialno.h"
#include "jtagtap.h"
#include "platforms/common/traceswo.h"
...
static bool cmd_sbw_scan(target *t, int argc, const char **argv);
...
/* custom intermittently-powered options */
static bool cmd_spi_test(target *t, int argc, const char **argv);
static bool cmd_pause_supply(target *t, int argc, const char **argv);
static bool cmd_spi_voltage(target *t, int argc, const char **argv);
static bool cmd_get_voltage(target *t, int argc, const char **argv);
static bool cmd_set_energyguard(target *t, int argc, const char **argv);
static bool cmd_set_pre_bp_energyguard(target *t, int argc, const char **argv);
static bool cmd_clear_energyguard(target *t, int argc, const char **argv);
static bool cmd_int_count(target *t, int argc, const char **argv);
static bool cmd_wait_attach(target *t, int argc, const char **argv);
static bool cmd_interrupt_on_power_failure(target *t, int argc, const char **argv);
static bool cmd_current_architecture(target *t, int argc, const char **argv);
static bool cmd_set_passive_mode(target *t, int argc, const char **argv);
static bool cmd_set_profile_mode(target *t, int argc, const char **argv);
static bool cmd_set_test_mode(target *t, int argc, const char **argv);
static bool cmd_get_profile_time(target *t, int argc, const char **argv);
...
const command_t cmd_list[] = {
    ...
    {"sbw_scan", cmd_sbw_scan, "EXPERIMENTAL: Scan spy-bi-wire JTAG chain for devices" },
    ...
    { "spi_test", (cmd_handler)cmd_spi_test, "Send test message to over SPI Bus" },
    { "count_int", (cmd_handler)cmd_int_count, "Return number of power intermittencies" },
    { "pause_supply", (cmd_handler)cmd_pause_supply, "Pause PMU supply (enable|disable)" },
    { "spi_get_voltage", (cmd_handler)cmd_spi_voltage, "Request SPI to send current voltage" },
    { "voltage", (cmd_handler)cmd_get_voltage, "return voltage level" },
    { "eg", (cmd_handler)cmd_set_energyguard, "set an energyGuard based on the voltage just after breakpoint" },
    { "pre_bp_eg", (cmd_handler)cmd_set_pre_bp_energyguard, "set an energyGuard based around voltage just before breakpoint" },
    { "delete_eg", (cmd_handler)cmd_clear_energyguard, "Remove energy Guard" },
    { "wait_attach", (cmd_handler)cmd_wait_attach, "Wait before connecting until device is ready" },
    {"interrupt_on_power_failure", (cmd_handler)cmd_interrupt_on_power_failure, "Send SIGINT after a succeeded reconnection"},
    { "passive_mode", (cmd_handler)cmd_set_passive_mode, "EXPERIMENTAL: Profile mode (enable|disable)"},
    { "profile_mode", (cmd_handler)cmd_set_profile_mode, "EXPERIMENTAL: Profile mode (enable|disable)"},
    { "test_mode", (cmd_handler)cmd_set_test_mode, "EXPERIMENTAL: mode (int)"},
    { "profile_time", (cmd_handler)cmd_get_profile_time, "EXPERIMENTAL: Profile time"},
    { "current_architecture", (cmd_handler)cmd_current_architecture, "Returns current architecture"},
...
#define BOARD_IDENT "DIPS Probe" PLATFORM_IDENT FIRMWARE_VERSION
...
bool cmd_version(target *t, int argc, const char **argv)
{
    ...
    gdb_out(BOARD_IDENT);
    gdb_outf(", DIPS Intermittently-Powered Debugger\n");
    gdb_out("Copyright (C) 2022 ES TU Delft\n");
    gdb_out("Based on the Blackmagic Debug Probe\n");
...
static bool cmd_jtag_scan(target *t, int argc, const char **argv)
{
    ...
    gdb_out("Please use SWD for intermittent mode!\n");
...
bool cmd_swdp_scan(target *t, int argc, const char **argv)
{
    ...
    // We remove everything from last connections
    target_list_free_extended();

    uint32_t devs = 0;
    volatile struct exception e;
    // Apollo does fail sometimes... Let's just try it 8 times just to be sure :)
...
/**
 * Scan spy-bi-wire for devices
 * @param target *t
 */
bool cmd_sbw_scan(target *t, int argc, const char **argv)
{
    (void)t;
    volatile uint32_t targetid = 0;
    if(argc > 1)
...
{
    if(cmd_swdp_scan(t, argc, argv)){
        return true;
    }

    if( cmd_sbw_scan(t,argc,argv)){
        return true;
    }
...
bool parse_architecture(const char *value, arch_mode_t *out)
{
    const size_t value_len = strlen(value);
    if (value_len && !strncmp(value, "msp", value_len))
        *out = MSP_ARCH;
    else if (value_len && !strncmp(value, "arm", value_len))
        *out = ARM_ARCH;
    else {
        gdb_out("'msp' or 'arm' argument must be provided\n");
        *out = UNKNOWN_ARCH;
        return false;
    }
    return true;
...
bool parse_true_or_false(const char *value, bool *out)
{
    const size_t value_len = strlen(value);
    if (value_len && !strncmp(value, "true", value_len))
        *out = true;
    else if (value_len && !strncmp(value, "false", value_len))
        *out = false;
    else {
        gdb_out("'true' or 'false' argument must be provided\n");
        return false;
    }
    return true;
}
...
static bool cmd_pause_supply(target *t, int argc, const char **argv)
{
    ...
    if(argc == 1)
    {
        gdb_outf("Pause supply:  %s\n", pause ? "enabled" : "disabled");
        return true;
    }
    else if(argc == 2)
    {
        if(parse_enable_or_disable(argv[1], &pause))
        {
            spi_send_pause(pause);
        }
    }
    else
    {
...
static bool cmd_int(target *t, int argc, const char **argv)
{
    (void)t;
    bool print_status = false;
    if(argc == 1)
    ...
    {
        if(parse_enable_or_disable(argv[1], &intermittent_mode))
        {
            print_status = true;
        }
    }
    ...
    if(print_status)
    {
        gdb_outf("Intermittent mode: %s\n", intermittent_mode ? "enabled" : "disabled");
    }
    return true;
}
...
static bool cmd_set_energyguard(target *t, int argc, const char **argv)
{
    ...
    if(argc > 1)
        gdb_outf("not implemented\n");
    else 
        setEnergyGuard(false);
...
static bool cmd_interrupt_on_power_failure(target *t, int argc, const char **argv){
    ...
    if(argc == 2)
    {
        parse_enable_or_disable(argv[1], &m_interrupt_on_power_failure);
    }
    else
    {
        gdb_outf("Unrecognized command format\n");
...
static bool cmd_current_architecture(target *t, int argc, const char **argv){
    (void)t;
      if(argc == 1)
    {
        gdb_outf("Current architecture type (1=unknown,2arch,3=msp):  %d\n", m_current_architecture);
        return true;
    }
    else if(argc == 2)
    {
        parse_architecture(argv[1], &m_current_architecture);
    }
    else
    {
        gdb_outf("Unrecognized command format\n");
...
static bool cmd_wait_attach(target *t, int argc, const char **argv){
    ...
    //    gdb_outf("Waiting for connection...");

    // Wait for interrupt so we can continue connecting
    // Try with TX pin (5)
    if (gpio_get(USBUSART_PORT, USBUSART_RX_PIN)){
        gdb_outf("true");
        enable_supply();
        return true;
    }
...
static bool cmd_set_profile_mode(target *t, int argc, const char **argv)
{
    if(argc == 1)
    {
        gdb_outf("Profile mode:  %s\n", (m_debugger_mode == MODE_PROFILE) ? "enabled" : "disabled");
        return true;
    }
    else if(argc == 2)
    {
        bool enable_step = false;
        if(parse_enable_or_disable(argv[1], &enable_step))
        {            
            m_debugger_mode = enable_step;
        }
    }
    else
    {
...
static bool cmd_set_passive_mode(target *t, int argc, const char **argv)
{
    if(argc == 1)
    {
        gdb_outf("Passive mode:  %s\n", (m_debugger_mode == MODE_PASSIVE) ? "enabled" : "disabled");
        return true;
    }
    else if(argc == 2)
    {
        ...
        {            
            if(enable_step){
                gpio_set_mode(DEBUGGER_PINS_PORT, GPIO_MODE_INPUT,
                GPIO_CNF_INPUT_PULL_UPDOWN, DEBUGGER_TX_PIN);  
                setup_dips_irq();
                m_debugger_mode = MODE_PASSIVE;
            } else {
                m_debugger_mode = MODE_REGULAR;
                gpio_set_mode(DEBUGGER_PINS_PORT, GPIO_MODE_OUTPUT_10_MHZ,
                GPIO_CNF_OUTPUT_PUSHPULL, DEBUGGER_TX_PIN);
            }
        }
    }
    else
...
static bool cmd_set_test_mode(target *t, int argc, const char **argv)
{
    if(argc == 1)
    {
        gdb_outf("Test mode:  %d\n", m_debugger_mode);
        return true;
    }
    else if(argc == 2)
    {        
            m_debugger_mode = (operation_mode_t) argv[1];
    }
    else
    {
        gdb_outf("Unrecognized command format\n");
...
// Can't print uint64_t directly, so split it into two uint32_t
void split_uint64(uint64_t input, uint32_t *high, uint32_t *low) {
    *low = (uint32_t) (input % 1000000000);
    *high = (uint32_t) (input / 1000000000);
}
...
static bool cmd_get_profile_time(target *t, int argc, const char **argv)
{
    ...
    {   
        ...
        split_uint64(stopwatch_time_ms, &upperpart, &lowerpart);
        gdb_outf("time [ns]:  %u%u \n", upperpart, lowerpart);
        return true;
    } else if(argc == 2){
        uint32_t timeLargestUs = largest_time*timerIntervalNs /1000;
        uint32_t timeSmallestUs = smallest_time*timerIntervalNs /1000;
        gdb_outf("statistics time\nLargest time:\t%u\nSmallest time:\t%u\n", timeLargestUs, timeSmallestUs);
        gdb_outf("Cycles:\t\t%u\n", cycles);
        return true;
    }
    else if(argc == 3)
