#include "general.h"
#include "platforms/common/usbuart.h"
#include "morse.h"
#include "platforms/common/usb.h"
#include "spi.h"
#include "platforms/common/aux_serial.h"
#include "gdb_main.h"
...
void platform_init(void){
    ...
    gpio_set_mode(BUS_IO_EN_PORT, GPIO_MODE_OUTPUT_2_MHZ,
                  GPIO_CNF_OUTPUT_PUSHPULL,
                  BUS_IO_EN_PIN | BUS_INT_EN_PIN);

    gpio_set(BUS_IO_EN_PORT, BUS_IO_EN_PIN);
    gpio_set(BUS_IO_EN_PORT, BUS_INT_EN_PIN);

    /* Configure P_INT as output for supply */
    gpio_set_mode(P_INT_PORT, GPIO_MODE_OUTPUT_10_MHZ,
                  GPIO_CNF_OUTPUT_PUSHPULL, P_INT_PIN);
    gpio_set_mode(P_INT_PORT, GPIO_MODE_INPUT,
                  GPIO_CNF_INPUT_FLOAT, P_INT_2_PIN);

    /* Configure RX and TX to the DUT */
    ...
    gpio_set_mode(DEBUGGER_PINS_PORT, GPIO_MODE_INPUT,
                  GPIO_CNF_INPUT_PULL_UPDOWN, DEBUGGER_RX_PIN);  
    setup_dips_irq();
    ...
    // We use ss pin for interrupt
//    gpio_set_mode(SPI_PORT, GPIO_MODE_OUTPUT_50_MHZ, GPIO_CNF_OUTPUT_ALTFN_PUSHPULL,
//                  SPI_SS_PIN);
    gpio_clear(SPI_PORT, SPI_SS_PIN);
...
uint16_t platform_dut_mv(void)
{
    uint32_t val = adc_read_regular(ADC1) * 99; /* 0-4095 */
    return (uint16_t) ((float)val / 81.910);
}
...
void profile_mode_pin_iqr_handler(uint16_t tx_value, uint16_t rx_value){
     if(!tx_value){
        // Device is not under JTAG control, ignore debugger updates 
        exti_reset_request(EXTI4);
        return;
    }

    // Check if ISR was triggered by fallingedge
    rx_value = gpio_get(DEBUGGER_PINS_PORT, DEBUGGER_RX_PIN);
    if (rx_value  != 0) {
        resume_stopwatch();
        enable_supply();
    } else  {
...
void passive_mode_pin_iqr_handler(uint16_t tx_value, uint16_t rx_value){
    if (rx_value  != 0 || tx_value != 0) {
...
void exti4_isr(void)
{
    // Only passive and profile mode need to interact with the dut pins directly
    if(m_debugger_mode != MODE_PROFILE && m_debugger_mode != MODE_PASSIVE){
        exti_reset_request(EXTI4);
        return;
    }

    uint16_t tx_value = gpio_get(DEBUGGER_PINS_PORT, DEBUGGER_TX_PIN);
    uint16_t rx_value = gpio_get(DEBUGGER_PINS_PORT, DEBUGGER_RX_PIN);

    switch (m_debugger_mode) {
        case MODE_PASSIVE:
            passive_mode_pin_iqr_handler(tx_value, rx_value);
            break;
        case MODE_PROFILE:
            profile_mode_pin_iqr_handler(tx_value, rx_value);
            break;
        default:
            break;
    }
...
void setup_dips_irq(void)
{
    nvic_enable_irq(NVIC_EXTI4_IRQ);
    nvic_set_priority(NVIC_EXTI4_IRQ, 1);

    exti_select_source(EXTI4, DEBUGGER_PINS_PORT);
    exti_set_trigger(EXTI4, EXTI_TRIGGER_BOTH);
    exti_enable_request(EXTI4);

    // enable timer for stopwatch
    init_stopwatch();
}
