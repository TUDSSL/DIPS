...
bool (*last_correct_target)(target *t);
bool saved_last_target = false;

#define PROBE(x)               \
   do {                        \
        last_correct_target = &(x); \
        if ((x)(t))                  \
            return true;          \
...
bool cortexm_probe(ADIv5_AP_t *ap)
{
   ...
   if (saved_last_target){
       do {
           if (last_correct_target(t))
               return true;
           target_check_error(t);
       } while (0);
   }
...
static enum target_halt_reason cortexm_halt_poll(target *t, target_addr_t *watch)
{
   ...
   switch (e.type) {
       case EXCEPTION_ERROR:
           return TARGET_HALT_INTERMITTENT;  // This is error or intermittent
