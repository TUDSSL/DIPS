...
enum target_intermittent intermittent_state = TARGET_NOT_INTERMITTENT;
...
/**
 * Checks if device is in intermittent state
 */
bool target_available(void)
{
    if (intermittent_state == TARGET_INTERMITTENT && intermittent_mode)
        return false;
    return true;
}
...
void target_lost(void){
    ...
    if (intermittent_mode)
        intermittent_state = TARGET_INTERMITTENT;
}

// Only when we detach the target we want to clear our cache as well
void target_list_free_extended(void){

    struct breakwatch* bw = bw_list;

    while (bw){
        struct breakwatch* bw_next = bw->next;
        free(bw);
        bw = bw_next;
    }
...
void target_list_free(void)
{
   ...
   while (target_list) {
       ...
       while (target_list->bw_list){
           struct breakwatch* bw = target_list->bw_list->next;
           free(target_list->bw_list);
           target_list->bw_list = bw;
       }
       free(target_list->target_storage);
       target_mem_map_free(target_list);
       free(target_list->tdesc);
       free(target_list);
       target_list = t;
   }
}
...
target *target_attach(target *t, struct target_controller *tc)
{
   ...
   t->attached = true;
   intermittent_state = TARGET_NOT_INTERMITTENT;
   return t;
}
...
static int copy_bw_to_heap(struct breakwatch* bws){
    struct breakwatch *bw = malloc(sizeof(struct breakwatch));

    memcpy(bw, bws, sizeof(struct breakwatch));
    bw->next = bw_list;
    bw_list = bw;
...
int target_restore_breakpoints(target *t){
    ...
    for (bw = bw_list; bw; bw = bw->next){
        target_breakwatch_set(t, bw->type, bw->addr, bw->size, false);
    }

    return 1;
}

static int remove_bw_from_heap(struct breakwatch* bws){

    struct breakwatch *bwp = NULL, *bw;
    for (bw = bw_list; bw; bwp = bw, bw = bw->next)
        if ((bw->type == bws->type) &&
           (bw->addr == bws->addr) &&
           (bw->size == bws->size))
            break;
...
int target_breakwatch_set(target *t,
                          enum target_breakwatch type, target_addr_t addr, size_t len, bool update_heap)
{
    struct breakwatch bw = {
        .type = type,
        .addr = addr,
